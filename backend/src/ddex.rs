...
pub struct DdexRegistration { pub isrc: String, pub iswc: Option<String> }

pub fn build_ern_xml(
    title: &str, isrc: &str, cid: &str,
    fp: &PatternFingerprint, wiki: &crate::wikidata::WikidataArtist,
) -> String {
  ...
  <ReleaseList>
    <Release>
      ...
      <ReleaseResourceReferenceList>
        ...
        isrc=isrc, title=title, cid=cid,
        band=fp.band, band_name=tier.as_str(), residue=fp.band_residue,
        prime=fp.mapped_prime, cycle=fp.cycle_position, dr=fp.digit_root,
...
pub async fn register(
    title: &str, isrc: &shared::types::Isrc, cid: &shared::types::BtfsCid,
    fp: &PatternFingerprint, wiki: &crate::wikidata::WikidataArtist,
) -> anyhow::Result<DdexRegistration> {
    let xml     = build_ern_xml(title, &isrc.0, &cid.0, fp, wiki);
    let ddex_url = std::env::var("DDEX_SANDBOX_URL")
        .unwrap_or_else(|_| "https://sandbox.ddex.net/ern".into());
    info!(isrc=%isrc, band=%fp.band, "Submitting ERN 4.1 to DDEX");
    if std::env::var("DDEX_DEV_MODE").unwrap_or_default() == "1" {
        warn!("DDEX_DEV_MODE=1 — stub"); return Ok(DdexRegistration { isrc: isrc.0.clone(), iswc: None });
    }
    let resp = reqwest::Client::new().post(&ddex_url)
        .header("Content-Type", "application/xml").body(xml).send().await?;
    if !resp.status().is_success() { anyhow::bail!("DDEX failed: {}", resp.status()); }
    Ok(DdexRegistration { isrc: isrc.0.clone(), iswc: None })
}
