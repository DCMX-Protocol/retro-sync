...
pub fn validate_eccn(eccn: &str) -> bool {
    if eccn == "EAR99" || eccn == "NLR" { return true; }
    let b = eccn.as_bytes();
    b.len() >= 5
        && b[0].is_ascii_digit()
...
impl Incoterm {
    pub fn code(&self) -> &'static str {
        match self {
            Self::Exw=>"EXW", Self::Fca=>"FCA", Self::Cpt=>"CPT",
            Self::Cip=>"CIP", Self::Dap=>"DAP", Self::Dpu=>"DPU",
            Self::Ddp=>"DDP", Self::Fas=>"FAS", Self::Fob=>"FOB",
            Self::Cfr=>"CFR", Self::Cif=>"CIF",
        }
    }
...
impl GtmsStore {
    ...
    pub fn save_classification(&self, r: ClassificationResult) {
        if let Ok(mut m) = self.classifications.lock() { m.insert(r.request_id.clone(), r); }
    }
    pub fn save_screening(&self, r: ScreeningResult) {
        if let Ok(mut m) = self.screenings.lock() { m.insert(r.screening_id.clone(), r); }
    }
    ...
    pub fn save_declaration(&self, d: ExportDeclaration) {
        if let Ok(mut m) = self.declarations.lock() { m.insert(d.declaration_id.clone(), d); }
    }
}
...
fn classify(req: &ClassificationRequest) -> ClassificationResult {
    ...
    let embargoed: Vec<String> = req.countries.iter()
        .filter(|c| EMBARGOED_COUNTRIES.contains(&c.as_str()))
        .cloned().collect();

    let restricted: Vec<String> = req.countries.iter()
    ...
    let licence_required = !embargoed.is_empty();
    let licence_exception = if !licence_required && eccn == "EAR99" {
        Some("NLR".into())  // No Licence Required
    } else { None };

    let incoterm = match req.product_type {
        ProductType::VinylRecord | ProductType::Cd | ProductType::Usb
            => Some(Incoterm::Dap),   // default for physical goods
        _   => None,
    };

    let notes = if embargoed.is_empty() && restricted.is_empty() {
        format!("EAR99 — no licence required for {} destination(s)", req.countries.len())
    } else if licence_required {
        format!("LICENCE REQUIRED for embargoed destination(s): {}. Do not ship/deliver.",
            embargoed.join(", "))
    } else {
        format!("Restricted territory review required: {}", restricted.join(", "))
    };
...
fn screen(req: &ScreeningRequest) -> ScreeningResult {
    ...
    let embargoed: Vec<String> = req.territories.iter()
        .filter(|t| EMBARGOED_COUNTRIES.contains(&t.as_str()))
        .cloned().collect();

    let restricted: Vec<String> = req.territories.iter()
        .filter(|t| RESTRICTED_TERRITORIES.contains(&t.as_str()))
        .cloned().collect();

    if EMBARGOED_COUNTRIES.contains(&req.payee_country.as_str()) {
        blocked.push(format!("Payee country '{}' is under comprehensive embargo", req.payee_country));
    }

    if !embargoed.is_empty() {
        blocked.push(format!("Delivery territories under embargo: {}", embargoed.join(", ")));
    }

    if !restricted.is_empty() {
        review.push(format!("Restricted territories require manual review: {}", restricted.join(", ")));
    }
...
pub async fn classify_work(
    ...
    state.audit_log.record(&format!(
        ...
        result.title, result.eccn, result.hs_code,
        result.licence_required, result.embargoed_countries,
    )).ok();
...
pub async fn get_declaration(
    ...
    state.gtms_db.get_declaration(&id)
        .map(Json)
