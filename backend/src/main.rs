...
mod gtms;      // Global Trade Management — ECCN, HS codes, export control, sanctions
...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .layer(middleware::from_fn_with_state(state.clone(), auth::verify_zero_trust))
        .with_state(state);
...
async fn upload_track(
        ...
        .map_err(|_| StatusCode::BAD_REQUEST)?
    {
        match field.name().unwrap_or("") {
            ...
            "audio"  => audio_bytes = field.bytes().await.map_err(|_| StatusCode::BAD_REQUEST)?.to_vec(),
            _        => {}
        }
    }
    ...
    let track_meta = dsp::TrackMeta {
        isrc: Some(isrc.0.clone()), upc: None,
        explicit: false, territory_rights: false,
        contributor_meta: false, cover_art_px: None,
    };
    ...
    state.audit_log.record(&format!(
        ...
        title, isrc, audio_bytes.len(), fp.band, tier.as_str(), qc_report.passed
    )).ok();
    ...
    if wikidata::isrc_exists(&isrc.0).await {
        ...
        state.mod_queue.add(moderation::ContentReport {
            ...
            resolved_at:  None, resolution: None, sla_hours: 24,
        });
    }
    ...
    if let Some(ref qid) = wiki.qid {
        ...
        state.audit_log.record(&format!(
            "WIKIDATA_ENRICH isrc='{}' artist='{}' qid='{}'", isrc, artist_name, qid
        )).ok();
    }
    ...
    // ── Pipeline ──────────────────────────────────────────────────────────
    let cid = btfs::upload(&audio_bytes, &title, &isrc).await
        .map_err(|_| StatusCode::BAD_GATEWAY)?;

    let tx_result = bttc::submit_distribution(&cid, &[], fp.band, None).await
        .map_err(|_| StatusCode::BAD_GATEWAY)?;
    let tx_hash = tx_result.tx_hash;

    let reg = ddex::register(&title, &isrc, &cid, &fp, &wiki).await
        .map_err(|_| StatusCode::BAD_GATEWAY)?;

    mirrors::push_all(&cid, &reg.isrc, &title, fp.band).await
        .map_err(|_| StatusCode::BAD_GATEWAY)?;
