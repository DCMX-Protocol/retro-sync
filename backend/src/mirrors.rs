...
pub async fn push_all(
    cid: &shared::types::BtfsCid, isrc: &str, title: &str, band: u8,
) -> anyhow::Result<()> {
    let (ia, bbs) = tokio::join!(
        push_internet_archive(cid, isrc, title, band),
        push_bbs(cid, isrc, title, band),
    );
    if let Err(e) = ia  { warn!(err=%e, "IA mirror failed"); }
    if let Err(e) = bbs { warn!(err=%e, "BBS mirror failed"); }
    Ok(())
}
...
async fn push_internet_archive(
    ...
    if access.is_empty() { warn!("ARCHIVE_ACCESS_KEY not set — skipping IA"); return Ok(()); }
    let tier       = RarityTier::from_band(band);
    let identifier = format!("retrosync-{}", isrc.replace('/', "-").to_lowercase());
    let url        = format!("https://s3.us.archive.org/{}/{}.meta.json", identifier, identifier);
    ...
    let resp = reqwest::Client::new().put(&url)
        ...
        .body(meta.to_string()).send().await?;
    if resp.status().is_success() { info!(isrc=%isrc, "Mirrored to IA"); }
    Ok(())
}
...
async fn push_bbs(
    ...
    reqwest::Client::builder().timeout(std::time::Duration::from_secs(15)).build()?
        .post(&url).json(&payload).send().await?;
