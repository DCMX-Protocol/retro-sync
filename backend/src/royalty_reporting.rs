...
pub enum CwrVersion { V21, V22 }
impl CwrVersion {
    pub fn as_str(self) -> &'static str { match self { Self::V21=>"02.10", Self::V22=>"02.20" } }
}
...
#[derive(Debug,Clone,Serialize,Deserialize)]
pub enum RecordingFormat { Audio, Visual, Audiovisual }
impl RecordingFormat {
    pub fn cwr_code(&self) -> &'static str {
        match self { Self::Audio=>"A", Self::Visual=>"V", Self::Audiovisual=>"AV" }
    }
}

#[derive(Debug,Clone,Serialize,Deserialize)]
pub enum RecordingTechnique { Analogue, Digital, Unknown }
impl RecordingTechnique {
    pub fn cwr_code(&self) -> &'static str {
        match self { Self::Analogue=>"A", Self::Digital=>"D", Self::Unknown=>"U" }
    }
}

#[derive(Debug,Clone,Serialize,Deserialize)]
pub enum MediaType { Cd, Vinyl, Cassette, Digital, Other }
impl MediaType {
    pub fn cwr_code(&self) -> &'static str {
        match self { Self::Cd=>"CD", Self::Vinyl=>"VI", Self::Cassette=>"CA",
                     Self::Digital=>"DI", Self::Other=>"OT" }
    }
}
...
impl Default for WorkRegistration {
    fn default() -> Self {
        Self {
            iswc: None, title: String::new(), language_code: "EN".into(),
            music_arrangement: "ORI".into(), text_music_relationship: "MTX".into(),
            excerpt_type: String::new(), composite_type: String::new(),
            version_type: "ORI".into(), writers: vec![], publishers: vec![],
            alternate_titles: vec![], performing_artists: vec![], recording: None,
            society: CollectionSociety::PrsUk, territories: vec![TerritoryScope::World],
            grand_rights_ind: false, composite_component_count: 0,
            date_of_publication: None, exceptional_clause: "U".into(),
            opus_number: None, catalogue_number: None, priority_flag: "N".into(),
        }
    }
}
...
pub fn generate_jasrac_jdisc_csv(works: &[WorkRegistration]) -> String {
    ...
    for w in works {
        let composer = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Composer|WriterRole::ComposerLyricist));
        let lyricist = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Lyricist));
...
pub fn generate_socan_metadata_json(works: &[WorkRegistration], sender_id: &str) -> String {
    let entries: Vec<serde_json::Value> = works.iter().map(|w| {
        let writers: Vec<serde_json::Value> = w.writers.iter().map(|wr| serde_json::json!({
...
pub fn generate_gema_csv(works: &[WorkRegistration]) -> String {
    ...
    for w in works {
        let comp = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Composer|WriterRole::ComposerLyricist));
        let text = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Lyricist));
...
pub fn generate_nordic_cwr_block(
    works: &[WorkRegistration],
    sender_id: &str,
) -> String {
    // Stamp all works with Nordic society territories and generate one CWR
    let nordic_works: Vec<WorkRegistration> = works.iter().map(|w| {
        let mut w2 = w.clone();
        w2.territories = vec![
...
pub fn generate_sacem_tsv(works: &[WorkRegistration]) -> String {
    let mut out = String::from(
        "ISWC\tTitre\tCompositeursIPI\tParoliersIPI\tEditeurIPI\tSociete\tLangue\r\n"
    );
    for w in works {
        let composers: Vec<&str> = w.writers.iter()
            .filter(|wr| matches!(wr.role, WriterRole::Composer|WriterRole::ComposerLyricist))
            .filter_map(|wr| wr.ipi_cae.as_deref()).collect();
        let lyricists: Vec<&str> = w.writers.iter()
...
pub fn generate_samro_csv(works: &[WorkRegistration]) -> String {
    let mut out = String::from(
        "ISWC,Title,Composer_IPI,Lyricist_IPI,Publisher_IPI,Language,Territory\r\n"
    );
    for w in works {
        let comp = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Composer|WriterRole::ComposerLyricist));
        let lyr  = w.writers.iter().find(|wr| matches!(wr.role, WriterRole::Lyricist));
        let pub_ = w.publishers.first();
        out.push_str(&format!(
...
pub fn generate_soundexchange_csv(rows: &[SoundExchangeRow]) -> String {
    ...
    for r in rows {
        out.push_str(&format!(
            "{},{},{},{},{},{:.2},{},{}\r\n",
            r.isrc, r.title, r.artist, r.album,
            r.play_count, r.royalty_usd, r.period_start, r.period_end,
        ));
    }
...
pub fn generate_mlc_csv(rows: &[MlcUsageRow], service_id: &str) -> String {
    ...
    for r in rows {
        out.push_str(&format!(
            "{},{},{},{},{},{},{:.2},{},{}\r\n",
            r.isrc, r.iswc.as_deref().unwrap_or(""),
            r.title, r.artist, r.service_name,
            r.play_count, r.royalty_usd, r.territory, r.period,
        ));
    }
...
pub fn generate_neighboring_rights_csv(rows: &[NeighboringRightsRow]) -> String {
    ...
    for r in rows {
        out.push_str(&format!(
            "{},{},{},{},{},{},{}\r\n",
            r.isrc, r.artist, r.label,
            r.play_count, r.territory, r.society, r.period,
        ));
    }
