...
impl SapConfig {
    pub fn from_env() -> Self {
        ...
        Self {
            ...
            ecc_idoc_url:      ev("SAP_ECC_IDOC_URL",      "http://ecc.retrosync.media:8000/sap/bc/idoc_xml"),
...
pub enum PostingStatus { Posted, Simulated, Failed, Disabled }
...
pub async fn post_royalty_document(
    ...
    if posting.amount < 0.0 || posting.net_amount < 0.0
    ...
    state.audit_log.record(&format!(
        ...
        posting.correlation_id, posting.payee_vendor_id,
        posting.amount, posting.amount_currency, cfg.dev_mode
    )).ok();
    ...
    if ecc_mode {
        ...
        let resp = state.sap_client.http
            .post(&cfg.ecc_idoc_url)
            .header("Content-Type", "application/xml")
            .body(idoc)
            .send().await
            .map_err(|e| { warn!(err=%e, "ECC IDoc POST failed"); StatusCode::BAD_GATEWAY })?;
    ...
    let resp = state.sap_client.http
        ...
        .json(&payload)
        .send().await
        .map_err(|e| { warn!(err=%e, "S/4HANA journal entry POST failed"); StatusCode::BAD_GATEWAY })?;
    ...
    let doc_no = body["d"]["CompanyCodeDocument"].as_str().map(str::to_string);
...
pub async fn sync_vendor(
    ...
    state.audit_log.record(&format!(
        ...
        vendor.legal_name, cfg.dev_mode
    )).ok();
    ...
    let resp = req
        ...
        .json(&payload)
        .send().await
        .map_err(|e| { warn!(err=%e, "S/4HANA BP upsert failed"); StatusCode::BAD_GATEWAY })?;
    ...
    let bp = body["d"]["BusinessPartner"].as_str()
        .or(vendor.bp_number.as_deref()).unwrap_or("").to_string();

    info!(bp=%bp, "S/4HANA vendor synced");
    Ok(Json(VendorSyncResult { bp_number: bp, status: "OK".into(), dev_mode: false }))
}
...
pub async fn emit_royalty_idoc(
    ...
    state.audit_log.record(&format!(
        "SAP_IDOC_EMIT corr='{}' dev_mode={}", posting.correlation_id, cfg.dev_mode
    )).ok();
    ...
    let resp = state.sap_client.http
        ...
        .body(idoc)
        .send().await
        .map_err(|e| { warn!(err=%e, "ECC IDoc emit failed"); StatusCode::BAD_GATEWAY })?;
