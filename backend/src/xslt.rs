...
/// Uses `xrust` for pure-Rust XSLT 1.0 processing.
pub fn apply_xslt(xml_input: &str, xsl_filename: &str) -> anyhow::Result<String> {
    ...
    let make_doc = |s: &str| -> Result<RNode, Error> {
    ...
    let src_doc = make_doc(xml_input).map_err(|e| anyhow::anyhow!("XML parse error: {:?}", e))?;
    let style_doc = make_doc(&xsl_src).map_err(|e| anyhow::anyhow!("XSL parse error: {:?}", e))?;

    let mut ctxt = from_document(style_doc, None, make_doc, |_| Ok(String::new()))
        .map_err(|e| anyhow::anyhow!("XSLT compile error: {:?}", e))?;

    ctxt.context(vec![Item::Node(src_doc)], 0);
    ...
    let seq = ctxt
        .evaluate(&mut sc)
        .map_err(|e| anyhow::anyhow!("XSLT eval error: {:?}", e))?;

    let output = seq.to_xml();
