...
use quick_xml::{Writer as XmlWriter, events::{BytesEnd, BytesStart, BytesText, Event}};
...
use crate::royalty_reporting::{
    CollectionSociety, WorkRegistration, Writer, Publisher,
    AlternateTitle, PerformingArtist, RecordingDetail,
};
...
fn text_elem(writer: &mut XmlWriter<Cursor<&mut Vec<u8>>>, tag: &str, value: &str) -> anyhow::Result<()> {
...
fn write_work(writer: &mut XmlWriter<Cursor<&mut Vec<u8>>>, work: &WorkRegistration) -> anyhow::Result<()> {
    ...
    text_elem(writer, "cwr:GrandRightsInd",    if work.grand_rights_ind { "Y" } else { "N" })?;
    text_elem(writer, "cwr:ExceptionalClause", &work.exceptional_clause)?;
    text_elem(writer, "cwr:OpusNumber",        work.opus_number.as_deref().unwrap_or(""))?;
    text_elem(writer, "cwr:CatalogueNumber",   work.catalogue_number.as_deref().unwrap_or(""))?;
    ...
    if !work.alternate_titles.is_empty() {
        ...
        for alt in &work.alternate_titles {
            ...
            text_elem(writer, "cwr:Language",  alt.language.as_deref().unwrap_or(""))?;
    ...
    if !work.performing_artists.is_empty() {
        ...
        for pa in &work.performing_artists {
            ...
            text_elem(writer, "cwr:FirstName", pa.first_name.as_deref().unwrap_or(""))?;
    ...
    if let Some(rec) = &work.recording {
        ...
        text_elem(writer, "cwr:ReleaseTitle", rec.release_title.as_deref().unwrap_or(""))?;
        text_elem(writer, "cwr:Label",        rec.label.as_deref().unwrap_or(""))?;
        text_elem(writer, "cwr:ReleaseDate",  rec.release_date.as_deref().unwrap_or(""))?;
...
fn write_writer(writer: &mut XmlWriter<Cursor<&mut Vec<u8>>>, w: &Writer) -> anyhow::Result<()> {
    ...
    text_elem(writer, "cwr:Society",   w.society.as_ref().map(|s| s.cwr_code()).unwrap_or(""))?;
    text_elem(writer, "cwr:Controlled", if w.controlled { "Y" } else { "N" })?;
    writer.write_event(Event::End(BytesEnd::new("cwr:Writer")))?;
    Ok(())
}

fn write_publisher(writer: &mut XmlWriter<Cursor<&mut Vec<u8>>>, p: &Publisher) -> anyhow::Result<()> {
    ...
    text_elem(writer, "cwr:Society",       p.society.as_ref().map(|s| s.cwr_code()).unwrap_or(""))?;
...
/// Uses `xot` for pure-Rust XSLT 1.0 processing.
pub fn apply_xslt(xml_input: &str, xsl_filename: &str) -> anyhow::Result<String> {
    ...
    let mut processor = xot::Xot::new();
    let source   = processor.parse(xml_input)?;
    let style    = processor.parse(&xsl_src)?;
    let result   = processor.transform(style, source, &[])?;
    let output   = processor.to_string(result)?;
    info!(stylesheet=%xsl_filename, "XSLT transform applied");
    Ok(output)
}
...
pub async fn transform_submission(
    ...
    let (society, xsl_file) = resolve_society(&society_slug)
        .ok_or_else(|| {
    ...
    state.audit_log.record(&format!(
        "XSLT_TRANSFORM society='{}' works={}", society.display_name(), works.len()
    )).ok();
...
pub async fn transform_all_submissions(
    ...
    for slug in slugs {
        ...
        match apply_xslt(&canonical, xsl_file) {
            Ok(xml) => { results.insert(slug.to_string(), serde_json::Value::String(xml)); }
            Err(e)  => {
                warn!(slug=%slug, err=%e, "XSLT failed for society");
                results.insert(slug.to_string(), serde_json::Value::String(format!("ERROR: {e}")));
            }
        }
    }

    state.audit_log.record(&format!(
        "XSLT_TRANSFORM_ALL works={} societies={}", works.len(), slugs.len()
    )).ok();
