...
impl ZkProofCache {
    pub fn open(path: &str) -> anyhow::Result<Self> {
        Ok(Self { cache: Mutex::new(HashMap::new()), path: path.to_string() })
    }
    pub fn cache_key(band: u8, n_artists: u32, total_btt: u64, splits_bps: &[u16]) -> [u8; 33] {
        ...
        for bps in splits_bps { h.update(bps.to_le_bytes()); }
    ...
    pub fn put(&self, band: u8, n_artists: u32, total_btt: u64, splits_bps: &[u16], proof: Vec<u8>) {
        let key = Self::cache_key(band, n_artists, total_btt, splits_bps);
        if let Ok(mut m) = self.cache.lock() { m.insert(key.to_vec(), proof); }
    }
