...
use crate::master_pattern::{digit_root, band_from_digit_root};
...
pub fn parse_b(text: &str) -> u64 {
    text.chars().map(|c| {
        ...
        v.to_string().chars().filter_map(|d| d.to_digit(10)).map(|d| d as u64).sum::<u64>()
    }).sum()
}
...
pub fn analyse(text: &str) -> AlphabetAnalysis {
    ...
    AlphabetAnalysis {
        text: text.to_string(),
        parse_a_sum: a_sum, parse_a_dr: a_dr,
        parse_b_sum: b_sum, parse_b_dr: b_dr,
        band,
...
pub fn resonance_report(artist: &str, title: &str, track_band: u8) -> ResonanceReport {
    ...
    ResonanceReport {
        artist: a, title: t, track_band,
        title_resonant, artist_resonant,
