...
const fn sum_arr7(a: [u64; 7]) -> u64 { a[0]+a[1]+a[2]+a[3]+a[4]+a[5]+a[6] }
const fn sum_arr4(a: [u64; 4]) -> u64 { a[0]+a[1]+a[2]+a[3] }
...
pub enum RarityTier { Common, Rare, Legendary }

impl RarityTier {
    pub fn from_band(band: u8) -> Self {
        match band { 0 => Self::Common, 1 => Self::Rare, _ => Self::Legendary }
    }
...
pub fn classify_prime(p: u64) -> Option<u8> {
    if BAND0.contains(&p) { return Some(0); }
    if BAND1.contains(&p) { return Some(1); }
    if BAND2.contains(&p) { return Some(2); }
    None
}

/// Map a band to its first (lowest) prime.
pub fn map_to_band_prime(band: u8) -> u64 {
    match band { 0 => 2, 1 => 19, _ => 41 }
}

/// Determine band from digit root.
pub fn band_from_digit_root(dr: u64) -> u8 {
    match dr { 4 => 0, 3 => 1, 2 => 2, _ => 0 }
}
...
pub fn pattern_fingerprint(isrc_bytes: &[u8], audio_hash: &[u8; 32]) -> PatternFingerprint {
    ...
    let expanded = {
        let s: u64 = hash_u64.to_string().chars()
